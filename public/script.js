...
function setupMobileLayout() {
  ...
  // Always keep chatForm inside chat-panel — never move it out
  ...
  if (isMobile) {
    ...
    // Desktop: restore correct column order
...
function resizeCanvas() {
  ...
  // Use ceil to cover any fractional pixel — prevents the 1px gap on right/bottom
  const W = Math.ceil(r.width);
  const H = Math.ceil(r.height) || Math.ceil(W * 0.75);

  if (W === 0 || H === 0) return;

  S.dpr = window.devicePixelRatio || 1;

  const physW = Math.ceil(W * S.dpr);
  const physH = Math.ceil(H * S.dpr);

  // Skip if nothing changed (prevents flicker on scroll events)
  if (gameCanvas.width === physW && gameCanvas.height === physH) {
    return;
  }

  // Preserve existing drawing as a full-resolution offscreen snapshot
  let snapshot = null;
  ...
  // Pin CSS size to exactly cover the wrap — no gaps, no overflow
  gameCanvas.style.width = '100%';
  gameCanvas.style.height = '100%';

  // Apply DPR scale so all drawing calls use CSS pixel coordinates
  ctx.setTransform(S.dpr, 0, 0, S.dpr, 0, 0);
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';

  // Fill white across the full physical canvas (bypass DPR transform)
  ctx.save();
  ctx.setTransform(1, 0, 0, 1, 0, 0);
  ctx.fillStyle = 'white';
  ctx.fillRect(0, 0, physW, physH);

  // Restore previous drawing scaled to new physical size
...
function onDrawStart(e) {
  ...
  if (S.tool === 'shape') {
      S.shapeStartX = pos.x;
      S.shapeStartY = pos.y;
      // Capture physical pixels directly (bypass DPR transform)
      ctx.save();
      ctx.setTransform(1, 0, 0, 1, 0, 0);
...
function onDrawMove(e) {
  ...
  if (S.tool === 'shape') {
      // Restore physical snapshot bypassing DPR transform
      ctx.save();
      ctx.setTransform(1, 0, 0, 1, 0, 0);
